...
public class BMenuBar extends MenuBar implements InterObjectCommunicatorEventListener {
  ...
  final String youtubeURL = "https://youtu.be/x_SNjAIZV1c", // "https://youtu.be/0gvja6lzhSw",
  ...
  private void init() {
    ...
    saveColorPattern.addActionListener(
        new ActionListener() {
          public synchronized void actionPerformed(ActionEvent e) {
            SavePattern(true);
          }
    ...
    saveProjectAs.addActionListener(
        new ActionListener() {
          public void actionPerformed(ActionEvent e) {
            SaveProject();
          }
  ...
  public void LoadProject(String myProjectFile, boolean updateCurrentProjectFilePath) {
    ...
    String selectedPalette =
        xmlHelper.GetDataFromXml("displaySettings.selectedPallette", xmlHelper.projectXML);
    ...
    try {
      imageController.flipImage =
          xmlHelper.GetIntFromXml("displaySettings.flipImage", xmlHelper.projectXML) == 1
              ? true
              : false;
      controlPanel.flipImageCheckboxPanel.checkbox.setSelected(
          xmlHelper.GetIntFromXml("displaySettings.flipImage", xmlHelper.projectXML) == 1
              ? true
              : false);
    ...
    //		try {
    //			//beadMaker.minBeadsComboBox = xmlHelper.GetIntFromXml("displaySettings.minimumBeads",
    // xmlHelper.projectXML);
    //
    ...
    try {
      // beadMaker.minBeadsComboBox = xmlHelper.GetIntFromXml("displaySettings.minimumBeads",
      // xmlHelper.projectXML);
      beadMaker.minBeadsComboBox.setSelectedItem(
          xmlHelper.GetDataFromXml("displaySettings.minimumBeads", xmlHelper.projectXML));
    } catch (ArrayIndexOutOfBoundsException e) {
      beadMaker.minBeadsComboBox.setSelectedIndex(0);
    }

    boolean perlerCheckBoxState =
        xmlHelper.GetIntFromXml("brandsToUse.perler", xmlHelper.projectXML) == 1 ? true : false;
    boolean artkalCheckBoxState =
        xmlHelper.GetIntFromXml("brandsToUse.artkalS", xmlHelper.projectXML) == 1 ? true : false;

    consoleHelper.PrintMessage(
        "brandsToUse.perler = "
            + xmlHelper.GetIntFromXml("brandsToUse.perler", xmlHelper.projectXML));
    consoleHelper.PrintMessage(
        "brandsToUse.artkalS = "
            + xmlHelper.GetIntFromXml("brandsToUse.artkalS", xmlHelper.projectXML));

    // if both brands are unchecked (which is an invalid state, but *could* happen,
    // check Perler by default
    if (!artkalCheckBoxState) {
      this.controlPanel.perlerCheckboxPanel.checkbox.setSelected(
          imageController.pallette.setBeadBrand("Perler", true));
      this.controlPanel.artkalCheckboxPanel.checkbox.setSelected(
          imageController.pallette.setBeadBrand("Artkal-S", false));
    } else if (!perlerCheckBoxState) {
      this.controlPanel.artkalCheckboxPanel.checkbox.setSelected(
          imageController.pallette.setBeadBrand("Artkal-S", true));
      this.controlPanel.perlerCheckboxPanel.checkbox.setSelected(
          imageController.pallette.setBeadBrand("Perler", false));
    } else {
      this.controlPanel.perlerCheckboxPanel.checkbox.setSelected(
          imageController.pallette.setBeadBrand("Perler", true));
      this.controlPanel.artkalCheckboxPanel.checkbox.setSelected(
    ...
    consoleHelper.PrintMessage(
        "controlPanel.perlerCheckboxPanel.checkbox = "
            + controlPanel.perlerCheckboxPanel.checkbox.isSelected());
    consoleHelper.PrintMessage(
        "controlPanel.artkalCheckboxPanel.checkbox = "
            + controlPanel.artkalCheckboxPanel.checkbox.isSelected());

    this.controlPanel.showGridCheckboxPanel.checkbox.setSelected(
        xmlHelper.GetIntFromXml("displaySettings.showGrid", xmlHelper.projectXML) == 1
            ? true
            : false);
    imageController.renderLabel.showGrid =
        xmlHelper.GetIntFromXml("displaySettings.showGrid", xmlHelper.projectXML) == 1
            ? true
            : false;

    this.controlPanel.renderPixelsAsBeadsCheckboxPanel.checkbox.setSelected(
        xmlHelper.GetIntFromXml("displaySettings.showPixelsAsBeads", xmlHelper.projectXML) == 1
            ? true
            : false);
    imageController.setRenderPixelsAsBeads(
        xmlHelper.GetIntFromXml("displaySettings.showPixelsAsBeads", xmlHelper.projectXML) == 1
            ? true
            : false);
    ...
    imageController.pallette.excludePearls =
        xmlHelper.GetIntFromXml("beadsToExclude.pearls", xmlHelper.projectXML) == 1
            ? ExcludePearls.TRUE
            : ExcludePearls.FALSE;
    controlPanel.excludePearlsCheckboxPanel.checkbox.setSelected(
        xmlHelper.GetIntFromXml("beadsToExclude.pearls", xmlHelper.projectXML) == 1 ? true : false);

    imageController.pallette.excludeTranslucents =
        xmlHelper.GetIntFromXml("beadsToExclude.translucents", xmlHelper.projectXML) == 1
            ? ExcludeTranslucents.TRUE
            : ExcludeTranslucents.FALSE;
    controlPanel.excludeTranslucentsCheckboxPanel.checkbox.setSelected(
        xmlHelper.GetIntFromXml("beadsToExclude.translucents", xmlHelper.projectXML) == 1
            ? true
            : false);

    if (useAppData) {
      imageController.pallette.GetPalletteFromXml(
          System.getenv("APPDATA")
              ...
              + "pallettes"
              + File.separator
              + controlPanel.customPalletteFiles[0][controlPanel.customPallette.getSelectedIndex()],
          xmlHelper);
    } else {
      imageController.pallette.GetPalletteFromXml(
    ...
    imageController.pallette.uncheckedColorIndices =
        ArrayHelper.SplitStringToIntegerArray(
            xmlHelper.GetDataFromXml("beadsToExclude.uncheckedColorIndices", xmlHelper.projectXML));
    ...
    for (int i = 0; i < imageController.pallette.perlerColorsRGB.length; i++) {
      for (int j = 0; j < imageController.pallette.uncheckedColorIndices.length; j++) {
        if (imageController.pallette.uncheckedColorIndices[j]
            == imageController
                .pallette
                .perlerColorsRGB[i][imageController.pallette.arrayIndex04_ColorIndex]) {
          consoleHelper.PrintMessage(
              "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ UNCHECKED MATCH $$$$$$$$$$$$");
          imageController
                  .pallette
                  .perlerColorsRGB[i][imageController.pallette.arrayIndex16_IsChecked] =
              0;
        }
      }
    }
    ...
    if (updateCurrentProjectFilePath) {
      xmlHelper.AlterXML(
          "currentProjectFilePath",
          myProjectFile.substring(0, myProjectFile.lastIndexOf(File.separator)),
          configFilePath,
          useAppData,
          appDataFolderName);

      beadMaker.windowController.setTitle(
          "--Bead Maker-- "
              + myProjectFile
              + "  --"
              + imageFile.substring(imageFile.lastIndexOf(File.separator) + 1)
              + "--");
    }
    // reload the XML into the variable
    xmlHelper.configXML = xmlHelper.GetXMLFromFile(configFilePath);
  ...
  public void SaveProject(String myProjectName) {
    ...
    if (!projectFile.exists()) {
      ...
      if (selectedFile != null) {
        ...
        consoleHelper.PrintMessage(
            "myProjectName EXTENSION = ***" + fileHelper.getExtension(myProjectName) + "***");
    ...
    xmlData[1] =
        new String[] {"dialValues.red", Integer.toString(controlPanel.sliderRed.getValue())};
    xmlData[2] =
        new String[] {"dialValues.green", Integer.toString(controlPanel.sliderGreen.getValue())};
    xmlData[3] =
        new String[] {"dialValues.blue", Integer.toString(controlPanel.sliderBlue.getValue())};
    xmlData[4] =
        new String[] {
          "dialValues.brightness", Integer.toString(controlPanel.sliderBrightness.getValue())
        };
    xmlData[5] =
        new String[] {
          "dialValues.contrast", Integer.toString(controlPanel.sliderContrast.getValue())
        };
    xmlData[6] =
        new String[] {
          "dialValues.saturation", Integer.toString(controlPanel.sliderSaturation.getValue())
        };
    xmlData[7] =
        new String[] {
          "dialValues.ditherLevel", Integer.toString(controlPanel.sliderDither.getValue())
        };
    xmlData[8] =
        new String[] {
          "dialValues.sharpness", Integer.toString(controlPanel.sliderSharpness.getValue())
        };
    xmlData[9] =
        new String[] {
          "dialValues.imageScale", Integer.toString(controlPanel.sliderScale.getValue())
        };
    xmlData[10] =
        new String[] {"dialValues.zoom", Integer.toString(controlPanel.sliderZoom.getValue())};
    xmlData[11] =
        new String[] {
          "brandsToUse.perler",
          Integer.toString(controlPanel.perlerCheckboxPanel.checkbox.isSelected() ? 1 : 0)
        };
    xmlData[12] = new String[] {"brandsToUse.hama", Integer.toString(0)};
    xmlData[13] =
        new String[] {
          "brandsToUse.artkalS",
          Integer.toString(controlPanel.artkalCheckboxPanel.checkbox.isSelected() ? 1 : 0)
        };

    xmlData[14] =
        new String[] {
          "beadsToExclude.translucents",
          Integer.toString(
              controlPanel.excludeTranslucentsCheckboxPanel.checkbox.isSelected() ? 1 : 0)
        };
    xmlData[15] =
        new String[] {
          "beadsToExclude.pearls",
          Integer.toString(controlPanel.excludePearlsCheckboxPanel.checkbox.isSelected() ? 1 : 0)
        };
    xmlData[16] =
        new String[] {
          "beadsToExclude.uncheckedColorIndices", imageController.pallette.getUncheckedIndices()
        };

    // xmlData[17] = new String[]
    // {"displaySettings.selectedPallette",Integer.toString(controlPanel.customPallette.getSelectedIndex())};
    xmlData[17] =
        new String[] {
          "displaySettings.selectedPallette",
          controlPanel.customPalletteFiles[0][controlPanel.customPallette.getSelectedIndex()]
        };
    xmlData[18] = new String[] {"displaySettings.showColorCodes", Integer.toString(0)};
    xmlData[19] =
        new String[] {
          "displaySettings.showPixelsAsBeads",
          Integer.toString(
              controlPanel.renderPixelsAsBeadsCheckboxPanel.checkbox.isSelected() ? 1 : 0)
        };
    xmlData[20] =
        new String[] {
          "displaySettings.pegboardMode",
          Integer.toString(controlPanel.pegboardSize.getSelectedIndex())
        };
    xmlData[21] =
        new String[] {
          "displaySettings.showGrid",
          Integer.toString(controlPanel.showGridCheckboxPanel.checkbox.isSelected() ? 1 : 0)
        };
    xmlData[22] =
        new String[] {
          "displaySettings.ditherMethod",
          Integer.toString(controlPanel.ditherMethod.getSelectedIndex())
        };
    xmlData[23] =
        new String[] {
          "displaySettings.backgroundColor.red",
          Integer.toString(imageController.renderScrollPanel.getViewport().getBackground().getRed())
        };
    xmlData[24] =
        new String[] {
          "displaySettings.backgroundColor.green",
          Integer.toString(
              imageController.renderScrollPanel.getViewport().getBackground().getGreen())
        };
    xmlData[25] =
        new String[] {
          "displaySettings.backgroundColor.blue",
          Integer.toString(
              imageController.renderScrollPanel.getViewport().getBackground().getBlue())
        };
    xmlData[26] =
        new String[] {
          "displaySettings.gridColor.red",
          Integer.toString(imageController.renderLabel.gridColor.getRed())
        };
    xmlData[27] =
        new String[] {
          "displaySettings.gridColor.green",
          Integer.toString(imageController.renderLabel.gridColor.getGreen())
        };
    xmlData[28] =
        new String[] {
          "displaySettings.gridColor.blue",
          Integer.toString(imageController.renderLabel.gridColor.getBlue())
        };
    xmlData[29] =
        new String[] {
          "displaySettings.flipImage",
          Integer.toString(controlPanel.flipImageCheckboxPanel.checkbox.isSelected() ? 1 : 0)
        };
    // xmlData[30] = new String[] {
    // "displaySettings.minimumBeads",Integer.toString(beadMaker.minBeadsComboBox.getSelectedIndex()) };
    xmlData[30] =
        new String[] {
          "displaySettings.minimumBeads", beadMaker.minBeadsComboBox.getSelectedItem().toString()
        };
    ...
    xmlHelper.AlterXML(
        "currentProjectFilePath",
        myProjectName.substring(0, myProjectName.lastIndexOf(File.separator)),
        configFilePath,
        useAppData,
        appDataFolderName);
    // reload the XML into the variable
    ...
    beadMaker.windowController.setTitle(
        "--Pixel Perfect-- "
            + myProjectName
            + "  --"
            + xmlData[0][1].substring(xmlData[0][1].lastIndexOf(File.separator) + 1)
            + "--");
  }
  ...
  public void onInterObjectCommunicator_CommunicateEvent(Object o) {
    if (o instanceof KeyEvent) {
      ...
      if ((e.getKeyCode() == KeyEvent.VK_M) && ((e.getModifiers() ^ KeyEvent.CTRL_MASK) == 0)) {
        consoleHelper.PrintMessage(
            "fired a keyPressed event from oCommInterface in MenuBar" + e.getKeyCode());
      ...
      if ((e.getKeyCode() == KeyEvent.VK_S)
          && ((e.getModifiers() ^ (KeyEvent.CTRL_MASK + KeyEvent.SHIFT_MASK)) == 0)) {
        SaveProject();
      }
