...
public class BMImage extends PImage {
  ...
  static final float scaleForPDFPrinting_Perler = 14.1741f,
  ...
  static final float Perler_RedPerceptionCorrectionFactor = 11, // 11,
  ...
  static final int DitherArrayIndex0_XPosition = 0,
  ...
  public BMImage(int width, int height, boolean myUseAppData, String myAppDataFolderName) {
    super(width, height);
    // have to do this because the PImage constructor sets format to RGB, which eliminates
    // transparency
    this.format = ARGB;
  ...
  public BMImage(
      int width, int height, BMImage colorMap, boolean myUseAppData, String myAppDataFolderName) {
    super(width, height);
    // have to do this because the PImage constructor sets format to RGB, which eliminates
    // transparency
    this.format = ARGB;
  ...
  //	//LoadInputImageFile
  //	//---------------------------------------------------------------------------
  //	public void LoadInputImageFile(WindowController windowController, RenderController
  // renderController, String imageFile) {
  ...
  //
  //			//imageScale_FinalRender = GetImageScale(originalCleanedImage, window_Render,
  // windowController);
  //			renderController.viewMode = RenderController.ViewMode.FULLRENDER_FULLCOLOR;
  //		} else {
  //			DialogBoxHelper.WarningDialog("The project's image file (" + imageFile + ") does not exist.
  // Please edit the PBP file to use a valid image file path.");
  //		}
  //		ConsoleHelper.PrintMessage("The method LoadInputImageFile is setting the redrawMode.
  // )))))))))))))))))))))))))))))))))))))))))))");
  //		renderController.SetRedrawMode(RenderController.redrawMode_Everything,
  // RenderController.ActiveDrawThread.TOOLBOX);
  //	}

  // ---------------------------------------------------------------------------
  // SplitImageIntoTiles
  // ---------------------------------------------------------------------------
  public static synchronized BMImage[] SplitImageIntoTiles(
      BMImage bmImage, RenderLabel renderLabel) {
  ...
  public synchronized int GetTileCountForImage(BMImage myImage, RenderLabel renderLabel) {
    return (int)
        (Math.ceil((float) myImage.width / renderLabel.pegboardPegsWide)
            * Math.ceil((float) myImage.height / renderLabel.pegboardPegsHigh));
  }
  ...
  void SavePNG() {
    ...
    // if (useAppData) {
    //	dataDir = new File(System.getenv("APPDATA") + File.separator + appDataFolderName,
    // File.separator);
    // } else {
    ...
    SynchronousJFXFileChooser chooser =
        new SynchronousJFXFileChooser(
            dataDir, "Portable Network Graphics (*.png)", new String[] {"png"});
  ...
  void SaveSCAD() {
    ...
    if (useAppData) {
      dataDir =
          new File(System.getenv("APPDATA") + File.separator + appDataFolderName, File.separator);
    } else {
      dataDir = new File(System.getProperty("user.dir"), File.separator);
    }
    ...
    SynchronousJFXFileChooser chooser =
        new SynchronousJFXFileChooser(dataDir, "OpenSCAD Designs (*.scad)", new String[] {"scad"});
    selectedFile = chooser.showSaveDialog();
      ...
      for (int y = 0; y < this.height; y++) {
        for (int x = 0; x < this.width; x++) {
          ...
          if (myPixelColor.getAlpha() == 255) {
            ...
            scadOutput.println(
                "color(["
                    + (myPixelColor.getRed() / 255.0f)
                    + ", "
                    + (myPixelColor.getGreen() / 255.0f)
                    + ", "
                    + (myPixelColor.getBlue() / 255.0f)
                    + "]) hollowCylinder(d=4.75, h=5, wallWidth=1.0);");
          }
        }
      }
    ...
    if (pixelAlpha == 255) { // (only get pixels with full alpha [255])
      ...
      for (int j = 0; j < palette.currentPalette.length; j++) {
        RGBDistance =
            (int)
                (Math.pow(
                        Math.abs(palette.currentPalette[j][palette.arrayIndex00_Red] - pixelR) + 1,
                        2)
                    + Math.pow(
                        Math.abs(palette.currentPalette[j][palette.arrayIndex01_Green] - pixelG)
                            + 1,
                        2)
                    + Math.pow(
                        Math.abs(palette.currentPalette[j][palette.arrayIndex02_Blue] - pixelB) + 1,
                        2));

        if (RGBDistance < BestRGBDistance) {
          BestRGBDistance = RGBDistance;
          // if we find a good match, set the colorPallette perlerColorsArrayIndex to the current
          // perlerColor
  ...
  public void SwapToPerlerColors(
      Palette palette,
      float colorMatchingWeight_DitherLevel,
      float colorMatchingWeight_R,
      float colorMatchingWeight_G,
      float colorMatchingWeight_B,
      float colorMatchingWeight_Saturation,
      float colorMatchingWeight_Contrast,
      float colorMatchingWeight_Brightness,
      float colorMatchingWeight_Sharpness,
      BMImage lutImage,
      int minBeads,
    ...
    if (isRecursiveCall) {
      consoleHelper.PrintMessage(
          "swapToPerlerColors_RecursionDepth = "
              + Integer.toString(++swapToPerlerColors_RecursionDepth));
    } else {
    ...
    float RedPerceptionCorrectionFactor,
    ...
    if (!GlobalConstants.showOriginalImageInsteadOfPixelMappedImage) {
      ...
      // loop through each pixel in the original image, and add its color to the colorPallette if it
      ...
      for (int i = 0; i < this.height * this.width; i++) {
        ...
        if (myPixelColor.getAlpha() == 255) { // (only get pixels with full alpha [255])
          ...
          for (int j = 0; j < palette.currentPalette.length; j++) {
            ...
            if (palette.currentPalette[j][palette.arrayIndex16_IsChecked] == 1
                && palette.currentPalette[j][palette.arrayIndex19_FailedMinBeadCheck] == 0) {
              ...
              rgbDistance =
                  (int)
                      (Math.pow(
                              Math.abs(
                                      palette.currentPalette[j][palette.arrayIndex13_MapRed]
                                          - pixelR)
                                  * 100
                          ...
                          + Math.pow(
                              Math.abs(
                                      palette.currentPalette[j][palette.arrayIndex14_MapGreen]
                                          - pixelG)
                                  * 100
                                  / GreenPerceptionCorrectionFactor,
                              2)
                          + Math.pow(
                              Math.abs(
                                      palette.currentPalette[j][palette.arrayIndex15_MapBlue]
                                          - pixelB)
                                  * 100
                                  / BluePerceptionCorrectionFactor,
                              2));
              if (rgbDistance < bestRGBDistance) {
                ...
                bestRGBDistance = rgbDistance;
                // if we find a good match, set the colorPallette perlerColorsArrayIndex to the
                // current perlerColor
                perlerColorsArrayIndex = j;
              }
            }
          ...
          // This process takes dither offsets from the current pixel and applies them to future
          // pixels.
          // It does *not* affect the coloring of the current pixel.
          // ----------------------------------------------------------------------------------
          int myDitherMethod = ditherMethod.ordinal();
          int ditherDivisor =
              ditherMatrix.ditherMatrix[myDitherMethod][0][DitherArrayIndex3_Denominator];

          for (int k = 0; k < ditherMatrix.ditherMatrix[myDitherMethod].length; k++) {
            // based on XPosition and YPosition values in DitherMatrices, target a specific pixel
            // for dithering positionally relative to the current pixel
            int targetPixel =
                i
                    + ditherMatrix.ditherMatrix[myDitherMethod][k][DitherArrayIndex0_XPosition]
                    + ditherMatrix.ditherMatrix[myDitherMethod][k][DitherArrayIndex1_YPosition]
                        * this.width;

            // if the forward dither is in bounds of the image,
            if (targetPixel < rDitherError.length) {
              // add the error to the forward pixel, adjusted by the error weight and the dither
              // slider (0-100)
              rDitherError[targetPixel] +=
                  (pixelR
                          - palette
                              .currentPalette[perlerColorsArrayIndex][palette.arrayIndex13_MapRed])
                      * colorMatchingWeight_DitherLevel
                      * ditherMatrix.ditherMatrix[myDitherMethod][k][DitherArrayIndex2_Numerator]
                      / ditherDivisor;
              gDitherError[targetPixel] +=
                  (pixelG
                          - palette
                              .currentPalette[perlerColorsArrayIndex][
                              palette.arrayIndex14_MapGreen])
                      * colorMatchingWeight_DitherLevel
                      * ditherMatrix.ditherMatrix[myDitherMethod][k][DitherArrayIndex2_Numerator]
                      / ditherDivisor;
              bDitherError[targetPixel] +=
                  (pixelB
                          - palette
                              .currentPalette[perlerColorsArrayIndex][palette.arrayIndex15_MapBlue])
                      * colorMatchingWeight_DitherLevel
                      * ditherMatrix.ditherMatrix[myDitherMethod][k][DitherArrayIndex2_Numerator]
                      / ditherDivisor;
            }
          ...
          if (palette.currentPalette[perlerColorsArrayIndex][palette.arrayIndex05_PixelCount]
              == 1) {
      ...
      if (!isRecursiveCall) {
        for (int i = 0; i < palette.currentPalette.length; i++) {
          if (palette.currentPalette[i][palette.arrayIndex16_IsChecked] == 1
      ...
      for (int i = 0; i < palette.currentPalette.length; i++) {
        ...
        // && palette.currentPalette[i][palette.arrayIndex05_PixelCount] > 0
          ...
          int beadCheckCount = palette.currentPalette[i][palette.arrayIndex05_PixelCount]; // +
      ...
      if (FailedMinBeadCheck_PerlerColorsArrayIndex > -1) {
        consoleHelper.PrintMessage(
            "FailedMinBeadCheckColors = (index:)"
                + Integer.toString(FailedMinBeadCheck_PerlerColorsArrayIndex)
                + " "
                + palette.perlerColorsNames[FailedMinBeadCheck_PerlerColorsArrayIndex][1]
                + " "
                + palette.perlerColorsNames[FailedMinBeadCheck_PerlerColorsArrayIndex][0]);
        palette
                .currentPalette[FailedMinBeadCheck_PerlerColorsArrayIndex][
                palette.arrayIndex19_FailedMinBeadCheck] =
            1;
        SwapToPerlerColors(
            palette,
            colorMatchingWeight_DitherLevel,
  ...
  // ---------------------------------------------------------------------------
  public synchronized BMImage HighlightSelectedColor(
      BMImage bmImage, Palette pallette, int myPalletteIndex) {
    ...
    for (int i = 0; i < pallette.currentPalette.length; i++) {
      if (pallette.currentPalette[i][pallette.arrayIndex04_ColorIndex] == myPalletteIndex) {
        colorIndex = i;
        consoleHelper.PrintMessage(
            "HighlightSelectedColor - color chosen: "
                + pallette
                    .perlerColorsNames[
                    pallette.currentPalette[i][pallette.arrayIndex04_ColorIndex]][1]
                + " "
                + pallette
                    .perlerColorsNames[
                    pallette.currentPalette[i][pallette.arrayIndex04_ColorIndex]][0]);
      }
    }

    // ConsoleHelper.PrintMessage("Looking for pallette index " + myPalletteIndex);
    // ConsoleHelper.PrintMessage("redValue = " +
    // pallette.currentPallette[colorIndex][pallette.arrayIndex13_MapRed	]);
    // ConsoleHelper.PrintMessage("grnValue = " +
    // pallette.currentPallette[colorIndex][pallette.arrayIndex14_MapGreen	]);
    // ConsoleHelper.PrintMessage("bluValue = " +
    // pallette.currentPallette[colorIndex][pallette.arrayIndex15_MapBlue	]);
    ...
    for (int i = 0; i < image.height * image.width; i++) {
      ...
      if (myPixelColor.getAlpha() == 255) {
        ...
        if (myPixelColor.getRed() == pallette.currentPalette[colorIndex][pallette.arrayIndex00_Red]
            && myPixelColor.getGreen()
                == pallette.currentPalette[colorIndex][pallette.arrayIndex01_Green]
            && myPixelColor.getBlue()
                == pallette.currentPalette[colorIndex][pallette.arrayIndex02_Blue]) {
          ...
          image.pixels[i] =
              ColorHelper.colorToProcessingColor(
                  new Color(
                      myPixelColor.getRed() / 4 + 175,
                      myPixelColor.getGreen() / 4 + 175,
                      myPixelColor.getBlue() / 4 + 175)); // subdued color
        }
      }
    }
  ...
  synchronized Color SaturationAdjust(Color myPixel, float colorMatchingWeight_Saturation) {
    ...
    if (colorMatchingWeight_Saturation > 0.0) {
      myRed +=
          (int)
              (colorMatchingWeight_Saturation
                  * distanceToFullSaturation
                  * ((myRed - avgVal) / (float) distanceToZeroSaturation));
      myGreen +=
          (int)
              (colorMatchingWeight_Saturation
                  * distanceToFullSaturation
                  * ((myGreen - avgVal) / (float) distanceToZeroSaturation));
      myBlue +=
          (int)
              (colorMatchingWeight_Saturation
                  * distanceToFullSaturation
                  * ((myBlue - avgVal) / (float) distanceToZeroSaturation));
    } else if (colorMatchingWeight_Saturation < 0.0) {
      myRed +=
          (int)
              (colorMatchingWeight_Saturation
                  * distanceToZeroSaturation
                  * ((myRed - avgVal) / (float) distanceToZeroSaturation));
      myGreen +=
          (int)
              (colorMatchingWeight_Saturation
    ...
    return new Color(
        ...
        MathHelper.ensureRange(myBlue, 0, 255),
        myAlpha);
  }
        ...
        for (int ky = -1; ky <= 1; ky++) {
          ...
          for (int kx = -1; kx <= 1; kx++) {
            ...
            if (valA == 255 && coreA == 255) {
              ...
              sumR +=
                  (kernel[ky + 1][kx + 1] * colorMatchingWeight_Sharpness + kernelPlusOne) * valR;
              sumG +=
                  (kernel[ky + 1][kx + 1] * colorMatchingWeight_Sharpness + kernelPlusOne) * valG;
              sumB +=
                  (kernel[ky + 1][kx + 1] * colorMatchingWeight_Sharpness + kernelPlusOne) * valB;
            } else {
  ...
  public void ColorCorrect(
    ...
    // for some reason pure black does not work properly, so this hack replaces color(0,0,0) with
    // color(1,1,1)
    // This is a hack fix for a known Processing bug:
    // https://github.com/processing/processing/wiki/Troubleshooting
    for (int i = 0; i < this.height * this.width; i++) {

      myPixel =
          ColorCorrect_SinglePixel(
              ColorHelper.pixeltoColor(this.pixels[i]),
              colorMatchingWeight_R,
              colorMatchingWeight_G,
              colorMatchingWeight_B,
              colorMatchingWeight_Saturation,
  ...
  // ColorCorrect_SinglePixel
  // --------------------------------------------------------------------------
  synchronized Color ColorCorrect_SinglePixel(
      Color myColor,
      float colorMatchingWeight_R,
      float colorMatchingWeight_G,
      float colorMatchingWeight_B,
    ...
    myRed =
        BrightnessAdjust(
            ContrastAdjust(
                (int) (myColor.getRed() * colorMatchingWeight_R - 128f),
                colorMatchingWeight_Contrast),
            colorMatchingWeight_Brightness);
    myGreen =
        BrightnessAdjust(
            ContrastAdjust(
                (int) (myColor.getGreen() * colorMatchingWeight_G - 128f),
                colorMatchingWeight_Contrast),
            colorMatchingWeight_Brightness);
    myBlue =
        BrightnessAdjust(
            ContrastAdjust(
                (int) (myColor.getBlue() * colorMatchingWeight_B - 128f),
                colorMatchingWeight_Contrast),
            colorMatchingWeight_Brightness);
  ...
  synchronized Color ApplyLUT(Color myColor, BMImage lutImage) {
    ...
    //
    //		ConsoleHelper.PrintMessage("Looking for LUT Pixel number: " + (((myBlue >> 2) * 4096) +
    // ((myGreen >> 2) * 64) + (myRed >> 2)));

    // if using a 512 x 512 LUT (64r x 64g x 64b), drop the two least significant bits in each of R,
    // G and B.
    // (red on X axis, green on Y axis, blue on Z axis)
    myLUTColor =
        ColorHelper.pixeltoColor(
            lutImage
                .pixels[
                ((myBlue >> 2) % 8 * 64)
                    + ((myBlue >> 2) / 8 * 64 * 512)
    ...
    return new Color(myLUTColor.getRed(), myLUTColor.getGreen(), myLUTColor.getBlue(), myAlpha);
  }
  ...
  public void MapColors() {
    ...
    //			bDitherError = new int[this.height * this.width];
    //
    //			//loop through each pixel in the original image, and add its color to the colorPallette if
    ...
    //							rgbDistance =
    //									(int)(
    //											Math.pow(Math.abs(pallette.currentPallette[j][pallette.arrayIndex13_MapRed		] -
    // pixelR) * 100 / RedPerceptionCorrectionFactor  , 2) +
    //											Math.pow(Math.abs(pallette.currentPallette[j][pallette.arrayIndex14_MapGreen	] -
    // pixelG) * 100 / GreenPerceptionCorrectionFactor, 2) +
    //											Math.pow(Math.abs(pallette.currentPallette[j][pallette.arrayIndex15_MapBlue	] -
    // pixelB) * 100 / BluePerceptionCorrectionFactor , 2)
    //											);
    ...
    //
    //					for (int k = 0; k < ditherMatrix.ditherMatrix[myDitherMethod].length; k++) {
    //						//based on XPosition and YPosition values in DitherMatrices, target a specific pixel for
    // dithering positionally relative to the current pixel
    //						int targetPixel =
    //								i +
    //								ditherMatrix.ditherMatrix[myDitherMethod][k][DitherArrayIndex0_XPosition] +
    //								ditherMatrix.ditherMatrix[myDitherMethod][k][DitherArrayIndex1_YPosition] *
    ...
    //							//add the error to the forward pixel, adjusted by the error weight and the dither
    // slider (0-100)
    //							rDitherError[targetPixel] += (pixelR -
    // pallette.currentPallette[perlerColorsArrayIndex][pallette.arrayIndex13_MapRed	]) *
    // colorMatchingWeight_DitherLevel *
    // ditherMatrix.ditherMatrix[myDitherMethod][k][DitherArrayIndex2_Numerator] / ditherDivisor;
    //							gDitherError[targetPixel] += (pixelG -
    // pallette.currentPallette[perlerColorsArrayIndex][pallette.arrayIndex14_MapGreen	]) *
    // colorMatchingWeight_DitherLevel *
    // ditherMatrix.ditherMatrix[myDitherMethod][k][DitherArrayIndex2_Numerator] / ditherDivisor;
    //							bDitherError[targetPixel] += (pixelB -
    // pallette.currentPallette[perlerColorsArrayIndex][pallette.arrayIndex15_MapBlue	]) *
    // colorMatchingWeight_DitherLevel *
    // ditherMatrix.ditherMatrix[myDitherMethod][k][DitherArrayIndex2_Numerator] / ditherDivisor;
    //						}
    //					}
    ...
    //					//if this is the first time we've encountered this perler color,
    //					if (pallette.currentPallette[perlerColorsArrayIndex][pallette.arrayIndex05_PixelCount] ==
    // 1) {
    //						//add 1 to the total number of colors used.
    //						pallette.totalPalletteColors++;
    //					}
