...
public class PDFHelper {
  ...
  // ---------------------------------------------------------------------------
  // SavePatternPDF
  // DONE: The built-in resize method blurs the resulting image-- but since we need a very specific
  // scale, try using our custom resize method to upsize, and then down-size using the built-in
  // resize method.
  ...
  public void SavePatternPDF(boolean fullColorPDFPrinting) {
    ...
    if (selectedFile != null) {
      ...
      consoleHelper.PrintMessage(
          "Number of tiles: "
              + imageController.originalCleanedImage.GetTileCountForImage(
                  imageController.originalCleanedImage, imageController.renderLabel));
      consoleHelper.PrintMessage(
          "Number of tiles: "
              + imageController.originalCleanedImage.GetTileCountForImage(
                  imageController.originalCleanedImage, imageController.renderLabel));
      ...
      if (fullColorPDFPrinting) {
        ...
        for (int h = 0; h < pallette.currentPalette.length; h++) {
          ...
          if (pallette.currentPalette[h][pallette.arrayIndex05_PixelCount] > 0
            ...
            // SavePatternPDF__localImage[h][0].save("F:\\My
            // Documents\\Dropbox\\Workspace\\BeadMaker2018\\poop\\SavePatternPDF__localImage_" + h
            // + ".png");
            ...
            for (int m = 0; m < SavePatternPDF__localImage[h].length; m++) {
              // have to use this temp image so that the BMImage.totalBeadsHightlighted doesn't get
              // defaulted to 0.
              // tempLocalImage = SavePatternPDF__localImage[h][m].HighlightSelectedColor(pallette,
              // h).get();
              // SavePatternPDF__beadCount[h][m] =
              // SavePatternPDF__localImage[h][m].totalBeadsHightlighted;
              // BMImage.HighlightSelectedColor(SavePatternPDF__localImage[h][m], pallette,
              // pallette.currentPallette[h][pallette.arrayIndex04_ColorIndex]).save("F:\\My
              // Documents\\Dropbox\\Workspace\\BeadMaker2018\\poop\\SavePatternPDF__localImage_" +
              // h + "_" + m + ".png");
              ...
              // SavePatternPDF__localImage[h][m].save("F:\\My
              // Documents\\Dropbox\\Workspace\\BeadMaker2018\\poop\\SavePatternPDF__localImage_" +
              // h + "_" + m + ".png");
              consoleHelper.PrintMessage(
      ...
      // tile iterator
      consoleHelper.PrintMessage(
          "Attempting to iterate over " + SavePatternPDF__pdfImage[0].length + " tiles.");
      for (int j = 0; j < SavePatternPDF__pdfImage[0].length; j++) {
        ...
        if (fullColorPDFPrinting) {
          ...
          // SavePatternPDF__pdfImage[k][j][0].save("F:\\My
          // Documents\\Dropbox\\Workspace\\BeadMaker2018\\poop\\SavePatternPDF__localImage_" + j +
          // "_" + k + ".png");
                ...
                if (SavePatternPDF__pdfImage[k][j][1] != null) {
                  ...
                  if (SavePatternPDF__pdfImage[k][j][1] != null) {
                    ...
                    DrawPDFFooter(
                        ...
                        // perlerColorsNames[currentPallette[k][arrayIndex04_ColorIndex]][0] + "
                        // (Beads: " + Integer.toString(SavePatternPDF__beadCount[k][j]) + ")",
                        "Tile "
                            + Integer.toString(j + 1)
                            + "\r\n"
                            + pallette
