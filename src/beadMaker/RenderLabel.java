...
public class RenderLabel extends JLabel
    implements MouseMotionListener, MouseListener, InterObjectCommunicatorEventListener {
    ...
    if (pixelX < imageWidth && pixelY < imageHeight) {
      ...
      Color pixelColor =
          ColorHelper.pixeltoColor(
              imageController.colorCorrectedBeadMappedImage.pixels[pixelX + (pixelY * imageWidth)]);

      if (pixelColor.getAlpha() == 255) {

        int myPalletteIndex =
            imageController.colorCorrectedBeadMappedImage.GetPerlerPaletteIndexForSinglePixel(
                pixelColor, imageController.pallette);
        ...
        String hoverText =
            imageController
                ...
                imageController
                    .pallette
                    .currentPalette[myPalletteIndex][
                    imageController.pallette.arrayIndex04_ColorIndex]][
                1];
        hoverText +=
            " "
                + imageController
            ...
            " (R: "
                + imageController
                    ...
                    imageController
                        ...
                        .currentPalette[myPalletteIndex][
                        imageController.pallette.arrayIndex04_ColorIndex]][
                    imageController.pallette.arrayIndex00_Red];
        hoverText +=
            " G: "
                + imageController
                    .pallette
                    ...
                    imageController
                        ...
                        .currentPalette[myPalletteIndex][
                        imageController.pallette.arrayIndex04_ColorIndex]][
                    imageController.pallette.arrayIndex01_Green];
        hoverText +=
            " B: "
                + imageController
                    .pallette
                    ...
                    imageController
                        ...
                        .currentPalette[myPalletteIndex][
                        imageController.pallette.arrayIndex04_ColorIndex]][
                    imageController.pallette.arrayIndex02_Blue];
        hoverText += ")";
        ...
        imageController.beadMaker.controlPanel.hoveredPixelColor.setForeground(
            ColorHelper.GetTextColorForBGColor(pixelColor));
  ...
  public void mouseEntered(MouseEvent e) {
    // DO NOTHING (has to be implemented for the MouseMotionListener interface, but we don't need
    // it)
  }
  ...
  public void paintComponent(Graphics g) {
        ...
        for (int y = 0; y < gridRows; y++) {
          ...
          g2d.draw(
              new Ellipse2D.Double(
                  x * imageWidth / gridColumns
                      + (imageWidth
                          / gridColumns
                          / 5f), // add 1/5 of the cell width to accommodate for stroke thickness
                  y * imageHeight / gridRows
                      + (imageHeight
                          / gridRows
                          / 5f), // add 1/5 of the cell width to accommodate for stroke thickness
    ...
    if (showGrid) {
      ...
      for (int x = 0; x < gridColumns; x++) {
        g2d.drawLine(
            x * imageWidth / gridColumns, 0, x * imageWidth / gridColumns, imageHeight - 1);
        // ConsoleHelper.PrintMessage("drawing line at " + (x * (imageWidth / gridColumns)) + ", 0,
        // " + (x * (imageWidth / gridColumns)) + ", " + (imageHeight - 1));
      }
      for (int y = 0; y < gridRows; y++) {
    ...
    if (dragPoint != null) {
      ...
      if (xPosition != xPositionLineEnd || yPosition != yPositionLineEnd) {
        ...
        StringHelper.drawString(
            g, xMessage + " " + yMessage, dragPoint.x + xDrawPos, dragPoint.y + yDrawPos, true);
      }
    }
  }
    ...
    if (o instanceof KeyEvent) {
      ...
      if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {
        consoleHelper.PrintMessage(
            "fired a keyPressed event from oCommInterface in RenderLabel" + e.getKeyCode());
        dragPoint = null;
        repaint();
      }
    }
