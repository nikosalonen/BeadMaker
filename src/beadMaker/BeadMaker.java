...
public class BeadMaker implements InterObjectCommunicatorEventListener {
  ...
  */
  ...
  public BeadMaker(String[] args) throws Exception {
    ...
    pallette =
        new Palette(
            xmlWorker.GetAbsoluteFilePathStringFromXml(
                "defaultPallette", xmlWorker.configXML, useAppData, appDataFolderName),
            xmlWorker,
            oComm,
            eventBus);

    imageController =
        new ImageController(this, pallette, windowController, oComm, useAppData, appDataFolderName);

    showAllColorsButtonPanel =
        new ShowAllColorsButtonPanel("<html>Show All <u>C</u>olors</html>", oComm);
    ...
    int[] minBeadsOptionsValues = {0, 5, 10, 25, 50, 100, 200, 500};

    String[] minBeadsOptions = {"0", "5", "10", "25", "50", "100", "200", "500"};

    ActionListener minBeadsActionListener =
        new ActionListener() {
    ...
    // ConsoleHelper.PrintMessage("renderPanel width = " +
    // Integer.toString(imageController.renderJPanel.getBounds().width));
    // ConsoleHelper.PrintMessage("renderPanel height = " +
    // Integer.toString(imageController.renderJPanel.getBounds().height));
    ...
    controlPanel =
        new ControlPanel(
            imageController, pallette, xmlWorker, oComm, useAppData, appDataFolderName);
    ...
    if (!loadedInputFile) {
      bMenuBar.LoadProject(
          xmlWorker.GetAbsoluteFilePathStringFromXml(
              "defaultProjectFilePath", xmlWorker.configXML, useAppData, appDataFolderName),
          false);
    }
    ...
    */
    ...
    // need to call this after the window has been packed, otherwise the render panel reports its
    // size as 0,0
