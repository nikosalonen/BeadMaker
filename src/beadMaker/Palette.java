...
public class Palette implements InterObjectCommunicatorEventListener {
  ...
  Color selectedColor =
      new Color(
          253, 254, 255); // Setting this to a random color that doesn't match any pallette colors.
  // (previously it was set to -1, which was a color match to Arkal White.
  ...
  public int totalPalletteColors = 0;

  public int totalBeadsUsed, // this counts beads in the full render OR selected tile
  ...
  //			palletteLabels.add(new PalletteLabel(perlerColorsNames[currentIndex][1] + " - " +
  // perlerColorsNames[currentIndex][0], color));
  //		}
  //	}
  ...
  public void checkUncheck(int colorIndex, boolean checkedState) {
    ...
    if (eventBus != null) {
      eventBus.publish("update images", Subscriber.IMAGE_CONTROLLER);
    }
  ...
  public void setSelectedColor(int colorIndex) {
    ...
    if (eventBus != null) {
      eventBus.publish(colorIndex, Subscriber.IMAGE_CONTROLLER);
    }
  }
  ...
  public void CreateButtons(String caller) {
    ...
    for (int i = 0; i < currentPalette.length; i++) {
      ...
      if (currentPalette[i][arrayIndex05_PixelCount] > 0) {
        palletteSubPanels.add(
            new PaletteSubPanel(
                perlerColorsNames[currentIndex][1]
                    + " "
                    + perlerColorsNames[currentIndex][0]
                    + " - "
                    + currentPalette[i][arrayIndex05_PixelCount],
                color,
                this,
                currentPalette[i][arrayIndex04_ColorIndex],
                true,
                oComm));
      }
    }
  ...
  public void GetPalletteFromXml(String fileName, XMLWorker xMLHelper) {
    ...
    for (int i = 0; i < colorXml.length; i++) {
      ...
      perlerColorsRGB[i][arrayIndex07_IsTranslucent] =
          colorXml[i].getChild("isTranslucent").getIntContent();
      perlerColorsRGB[i][arrayIndex08_IsNeutral] =
          colorXml[i].getChild("isNeutral").getIntContent();
      perlerColorsRGB[i][arrayIndex09_IsGrayscale] =
          colorXml[i].getChild("isGrayscale").getIntContent();
      perlerColorsRGB[i][arrayIndex10_Disabled] = colorXml[i].getChild("disabled").getIntContent();
      perlerColorsRGB[i][arrayIndex12_DisableGlobally] =
          colorXml[i].getChild("disabled").getIntContent();
      perlerColorsRGB[i][arrayIndex16_IsChecked] = 1;
      if (colorXml[i].getChild("sortOrder") != null) {
        perlerColorsRGB[i][arrayIndex17_SortOrder] =
            colorXml[i]
                .getChild("sortOrder")
                .getIntContent(); // if it doesn't exist, default to the ColorIndex instead
        // (backward compatibility with old palette XML files)
      } else {
      ...
      // custom pallettes use the mapRed, mapGreen and mapBlue values that differ from the red,
      // green, blue values,
      // but the default pallette does not, so it just uses the red, green, blue values for pixel
      // color mapping.
      if (colorXml[i].getChild("mapRed") != null) {
        perlerColorsRGB[i][arrayIndex13_MapRed] =
            colorXml[i].getChild("mapRed").getIntContent(); // destination: mapRed
        perlerColorsRGB[i][arrayIndex14_MapGreen] =
            colorXml[i].getChild("mapGreen").getIntContent(); // destination: mapGreen
        perlerColorsRGB[i][arrayIndex15_MapBlue] =
            colorXml[i].getChild("mapBlue").getIntContent(); // destination: mapBlue
      } else {
        perlerColorsRGB[i][arrayIndex13_MapRed] =
            colorXml[i].getChild("red").getIntContent(); // destination: mapRed
        perlerColorsRGB[i][arrayIndex14_MapGreen] =
            colorXml[i].getChild("green").getIntContent(); // destination: mapGreen
        perlerColorsRGB[i][arrayIndex15_MapBlue] =
            colorXml[i].getChild("blue").getIntContent(); // destination: mapBlue
      }
    }
  ...
  public void GetPalletteWithFiltersApplied() {
    ...
    // -------------------------------------------------------------------------------
    // disableGlobally: This is for colors that exist in the pallette but are not used in the
    // program (missing RGB values, etc.)
    for (int i = 0; i < perlerColorsRGB.length; i++) {
    ...
    consoleHelper.PrintMessage(
        "%%%%%%%%%%%%%%%%%%TOTAL PALLETTE COLORS = " + arrayLength + "%%%%%%%%%%%%%%%%%%");
  ...
  //
  //		selectedColor = new Color(253,254,255); //Setting this to a random color that doesn't match
  // any pallette colors. (previously it was set to -1, which was a color match to Arkal White.
  ...
  public void showAllColors() {
    ...
    for (int i = 0; i < palletteSubPanels.size(); i++) {
      palletteSubPanels
          .get(i)
          .paletteButton
          .setFont(new Font(labelFont.getName(), Font.BOLD, labelFont.getSize()));
      palletteSubPanels
          .get(i)
          .paletteButton
          .setForeground(
              ColorHelper.GetTextColorForBGColor(palletteSubPanels.get(i).paletteButton.bgColor));
    }
  }
        ...
        if (isFirstUncheckedIndex) {
          ...
          myUncheckedIndices =
              String.join(
                  ",",
                  myUncheckedIndices,
                  Integer.toString(currentPalette[i][arrayIndex04_ColorIndex]));
        }
      }
    }
  ...
  public void onInterObjectCommunicator_CommunicateEvent(Object o) {
    ...
    if (o instanceof ActionEvent) {
      if (o instanceof ActionEvent) {
        ...
        if (a.getSource() instanceof PaletteCheckBox) {
          ...
          consoleHelper.PrintMessage(
              "fired an event from oCommInterface in Palette: PaletteCheckBox action event");
    ...
    if (o instanceof KeyEvent) {
      ...
      if ((e.getKeyCode() == KeyEvent.VK_C) && ((e.getModifiers() & KeyEvent.CTRL_MASK) != 0)) {
        consoleHelper.PrintMessage(
            "fired a keyPressed event from oCommInterface in Palette" + e.getKeyCode());
        showAllColors();
      }
    }
