...
public class ImageController implements InterObjectCommunicatorEventListener {
  ...
  public BMImage originalCleanedImage, // #1
  ...
  public float colorMatchingWeight_R = 1.0f,
  ...
  ImageController(
    ...
    String imagePath =
        beadMaker.xmlWorker.GetAbsoluteFilePathStringFromXml(
            "imageFile", beadMaker.xmlWorker.projectXML, useAppData, appDataFolderName);
  ...
  public void setOriginalCleanedImage(String imagePath) {
    ...
    originalCleanedImage =
        new BMImage(
            imageHelper.getBufferedImageWithAlphaChannelFromURL(imagePath),
            useAppData,
            appDataFolderName);
    updateImages();
  }
  ...
  public void loadColorMap(String imagePath) {
    ...
    for (int i = 0; i < colorMap.width; i++) {
      ...
      for (int y = 0; y < colorMappedSprite.height; y++) {
        for (int x = 0; x < colorMappedSprite.width; x++) {
          for (int j = 0; j < colorMap.height; j++) {
            if (colorMappedSprite.pixels[y * colorMappedSprite.width + x]
                == colorMap.pixels[j * colorMap.width]) {
              colorMappedSprite.pixels[y * colorMappedSprite.width + x] =
                  colorMap.pixels[j * colorMap.width + i];
            }
          }
        }
      }
      colorMappedImage =
          (BMImage)
              imageHelper.ReplacePortionOfImage(
                  colorMappedImage, colorMappedSprite, i * colorMappedSprite.width, 0);
    }
    // colorMappedImage = (BMImage)ImageHelper.ReplacePortionOfImage(colorMappedImage,
    // originalCleanedImage, 0, 0);
  ...
  public void updateImages() {
    ...
    consoleHelper.PrintMessage(
        "colorCorrectedBeadMappedImage.width = " + colorCorrectedBeadMappedImage.width);

    if (flipImage) {
      colorCorrectedBeadMappedImage.FlipHorizontally();
    }
    colorCorrectedBeadMappedImage.ditherMethod =
        BMImage.DitherMethod.values()[this.ditherMethod.ordinal()];
    colorCorrectedBeadMappedImage.resize(
        (int) Math.floor(originalCleanedImage.width * colorMatchingWeight_ImageScale), 0);
    ...
    if (selectedColorIndex == -1) {
      ...
      // renderLabel needs a copy of the image for its PaintComponent method (when
      // renderPixelsAsBeads is turned on)
      renderLabel.renderImage = colorCorrectedBeadMappedImage.get();
    } else {
      colorCorrectedBeadMappedImage_SingleColor =
          colorCorrectedBeadMappedImage.HighlightSelectedColor(
              colorCorrectedBeadMappedImage, pallette, selectedColorIndex);
      perlerSwappedIcon =
          new ImageIcon((BufferedImage) colorCorrectedBeadMappedImage_SingleColor.getNative());
      // renderLabel needs a copy of the image for its PaintComponent method (when
      // renderPixelsAsBeads is turned on)
      renderLabel.renderImage = colorCorrectedBeadMappedImage_SingleColor.get();
    }

    ImageIcon zoomedIcon =
        new ImageIcon(
            perlerSwappedIcon
                .getImage()
                .getScaledInstance(
                    perlerSwappedIcon.getIconWidth() * zoomFactor / 100,
                    perlerSwappedIcon.getIconHeight() * zoomFactor / 100,
  ...
  //
  //		ConsoleHelper.PrintMessage("image dimensions: " + imageWidth + "px W x " + imageHeight + "px
  // H");
  //
  ...
  //
  //		//using the built-in resize function alone results in a blurred result, so we do a custom
  // resize below.
  ...
  //					//I don't understand what this line is doing, but I think it's beautiful. Somehow this is
  // upscaling the image.
  //					localImage.pixels[j + k * imageWidth * scale + i * scale * scale - i % imageWidth * scale *
  // (scale - 1)] = localCopyImage.pixels[i];
  //				}
  //			}
  //		}
  //		localImage.updatePixels();
  ...
  // ---------------------------------------------------------------------------
  public synchronized BMImage[] resizeImageforPDFOutput(
      BMImage image, ImageController.PegboardMode myPegboardMode, boolean mySplitSuperPegboard) {
    ...
    switch (myPegboardMode) {
      ...
      case LEGO_8x8:
        ...
        localImage[0].setPixels(
            pixelsContainer.pixels, pixelsContainer.width, pixelsContainer.height);
        localImage[0].resize(
            0,
            (int)
                Math.ceil(
                    localImage[0].height
                        * BMImage.scaleForPDFPrinting_Perler
                        / (float) BMImage.upscalerForPDFPrinting));
      ...
      case PERLER_SUPERPEGBOARD_PORTRAIT:
        ...
        outputImage[0] =
            localImage[0].get(
                0, 0, localImage[0].width, (int) Math.floor(localImage[0].height / 2.0f));
        outputImage[0].resize(
            (int)
                Math.ceil(
                    (float) localImage[0].width
                        * BMImage.scaleForPDFPrinting_Perler
                        / BMImage.upscalerForPDFPrinting),
            0);
        // get the bottom half of the image
        // localImage[0] = resizeImage(localImage[0], localImage[0].upscalerForPDFPrinting);
        pixelsContainer = imageHelper.resizeImage(localImage[1], BMImage.upscalerForPDFPrinting);
        localImage[1].setPixels(
            pixelsContainer.pixels, pixelsContainer.width, pixelsContainer.height);
        outputImage[1] =
            localImage[1].get(
                0,
                (int) Math.floor(localImage[1].height / 2.0f),
                localImage[1].width,
                (int) Math.ceil(localImage[1].height / 2.0f));
        outputImage[1].resize(
            (int)
                Math.ceil(
                    (float) localImage[1].width
                        * BMImage.scaleForPDFPrinting_Perler
                        / BMImage.upscalerForPDFPrinting),
            0);
        break;
      ...
      case PERLER_SUPERPEGBOARD_LANDSCAPE:
        ...
        outputImage[0] =
            localImage[0].get(
                0, 0, (int) Math.floor(localImage[0].width / 2.0f), localImage[0].height);
        outputImage[0].resize(
            0,
            (int)
                Math.ceil(
                    (float) localImage[0].height
                        * BMImage.scaleForPDFPrinting_Perler
                        / BMImage.upscalerForPDFPrinting));
        ...
        outputImage[1] =
            localImage[1].get(
                (int) Math.floor(localImage[1].width / 2.0f),
                0,
                (int) Math.floor(localImage[1].width / 2.0f),
                localImage[1].height);
        outputImage[1].resize(
      ...
      case PERLERMINI_FORPDFPRINTING:
        ...
        localImage[0].resize(
            ...
            (int)
                Math.ceil(
                    (float) localImage[0].height
                        * BMImage.scaleForPDFPrinting_PerlerMini
                        / BMImage.upscalerForPDFPrinting));
        outputImage[0] = localImage[0];
        outputImage[1] = null;
        break;
  ...
  public void setColorMatchingWeight(String name, int value) {
    switch (name) {
      ...
      case "Brightness":
        ...
        break;
      case "Contrast":
        this.colorMatchingWeight_Contrast =
            (float) ((Math.pow(4.0, Math.pow(4.0, (value + 50f) / 100f)) - 1.0f) / 15f);
        consoleHelper.PrintMessage(
            "colorMatchingWeight_Contrast = " + colorMatchingWeight_Contrast);
        break;
      case "Saturation":
      ...
      case "Sharpness":
        ...
        consoleHelper.PrintMessage(
            "colorMatchingWeight_Sharpness = " + Float.toString(colorMatchingWeight_Sharpness));
        break;
      case "Scale %":
  ...
  public void onInterObjectCommunicator_CommunicateEvent(Object o) {
    ...
    if (o instanceof KeyEvent) {
      ...
      if ((e.getKeyCode() == KeyEvent.VK_C) && ((e.getModifiers() & KeyEvent.CTRL_MASK) != 0)) {
        consoleHelper.PrintMessage(
            "fired a keyPressed event from oCommInterface in ImageController" + e.getKeyCode());
        showAllColors();
      }
    }
  }
