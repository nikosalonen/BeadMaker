...
public class ControlPanel extends JPanel implements InterObjectCommunicatorEventListener {
  ...
  ControlPanel(
    ...
    ActionListener ditherMethodActionListener =
        new ActionListener() {
          public void actionPerformed(ActionEvent actionEvent) {
            imageController.setDitherMethod(
                ImageController.DitherMethod.values()[ditherMethod.getSelectedIndex()]);
            pallette.CreateButtons("ditherMethodActionListener");
          }
        };

    ActionListener customPalletteActionListener =
        new ActionListener() {
          public void actionPerformed(ActionEvent actionEvent) {
            ...
            if (useAppData) {
              consoleHelper.PrintMessage(
                  System.getenv("APPDATA")
                      ...
                      + appDataFolderName
                      + File.separator
                      + customPalletteFiles[0][customPallette.getSelectedIndex()]);
              pallette.GetPalletteFromXml(
                  System.getenv("APPDATA")
                      + File.separator
                      + appDataFolderName
                      + File.separator
    ...
    ActionListener pegboardSizeActionListener =
        new ActionListener() {
          public void actionPerformed(ActionEvent actionEvent) {
    ...
    ActionListener lutActionListener =
        new ActionListener() {
          public void actionPerformed(ActionEvent actionEvent) {

            if (useAppData) {
              imageController.loadLUT(
                  System.getenv("APPDATA")
                      + File.separator
                      + appDataFolderName
                      + File.separator
                      + "LUTs"
                      + File.separator
                      + lutFiles[0][lutSelector.getSelectedIndex()]);
            } else {
              imageController.loadLUT(
                  System.getProperty("user.dir")
                      + File.separator
    ...
    // ControlPanelSubPanel palletteControlsPanel = new ControlPanelSubPanel(new Color(128,55,192),
    // BoxLayout.Y_AXIS);
    ...
    if (useAppData) {
      pallettePath =
          new File(System.getenv("APPDATA") + File.separator + appDataFolderName, "pallettes");
    } else {
    ...
    customPalletteFiles =
        new String[2][listOfPalletteFiles.length]; // index 0 = Filename, index 1 = common name
    ...
    FilenameFilter lutFileNameFilter =
        new FilenameFilter() {
    ...
    //			if (customPalletteFiles[i].equals(XMLHelper.GetDataFromXml("defaultPallette",
    // xmlHelper.configXML).replace("pallettes\\", ""))) {
    ...
    // palletteModeControlPanel.setLayout(new BoxLayout(palletteModeControlPanel,
    // BoxLayout.Y_AXIS));
    ...
    perlerCheckboxPanel =
        new CheckBoxPanel("<html><u>P</u>erler</html>", true, pallette, imageController, oComm);
    artkalCheckboxPanel =
        new CheckBoxPanel("<html><u>A</u>rtkal-S</html>", pallette, imageController, oComm);
    hamaCheckboxPanel = new CheckBoxPanel("Hama", pallette, imageController, oComm);

    showGridCheckboxPanel =
        new CheckBoxPanel("<html>Show <u>G</u>rid</html>", true, imageController, oComm);
    renderPixelsAsBeadsCheckboxPanel =
        new CheckBoxPanel(
            "<html>Show Pixels as <u>B</u>eads</html>", false, imageController, oComm);

    excludePearlsCheckboxPanel =
        new CheckBoxPanel("Exclude Pearls", true, pallette, imageController, oComm);
    excludeTranslucentsCheckboxPanel =
        new CheckBoxPanel("Exclude Translucents", true, pallette, imageController, oComm);

    bgColorButtonPanel = new ColorChangerButtonPanel("Background Color", imageController);
    gridColorButtonPanel =
        new ColorChangerButtonPanel("Grid Color", imageController, imageController.renderLabel);

    flipImageCheckboxPanel =
        new CheckBoxPanel(
            "<html><u>F</u>lip Image</html>", false, pallette, imageController, oComm);
    // flipImageCheckboxPanel 	= new CheckBoxPanel("Flip Image", false, pallette, imageController,
    // oComm);
    ...
    flipImageCheckboxPanelBottom.setLayout(
        new BoxLayout(flipImageCheckboxPanelBottom, BoxLayout.X_AXIS));
    flipImageCheckboxPanelBottom.setOpaque(false);

    ControlPanelComboPanel flipImageCheckboxHorizontalPanelLeft =
        new ControlPanelComboPanel(new Color(160, 160, 160), false);
    ControlPanelComboPanel flipImageCheckboxHorizontalPanelRight =
        new ControlPanelComboPanel(new Color(160, 160, 160), false);
    ...
    palletteCheckboxPanelBottom.setLayout(
        new BoxLayout(palletteCheckboxPanelBottom, BoxLayout.X_AXIS));
    palletteCheckboxPanelBottom.setOpaque(false);

    ControlPanelComboPanel palletteCheckboxHorizontalPanelLeft =
        new ControlPanelComboPanel(new Color(160, 160, 160), false);
    ControlPanelComboPanel palletteCheckboxHorizontalPanelRight =
        new ControlPanelComboPanel(new Color(160, 160, 160), false);
    ...
    pegboardCheckboxPanelBottom.setLayout(
        new BoxLayout(pegboardCheckboxPanelBottom, BoxLayout.X_AXIS));
    pegboardCheckboxPanelBottom.setOpaque(false);

    ControlPanelComboPanel pegboardCheckboxHorizontalPanelLeft =
        new ControlPanelComboPanel(new Color(160, 160, 160), false);
    ControlPanelComboPanel pegboardCheckboxHorizontalPanelRight =
        new ControlPanelComboPanel(new Color(160, 160, 160), false);
    ...
    RandomizeParametersButton randomizeParametersButton_Backward =
        new RandomizeParametersButton("Backward", new Color(204, 204, 204), this, BACKWARD, oComm);
    RandomizeParametersButton randomizeParametersButton_Forward =
        new RandomizeParametersButton("Forward", new Color(204, 204, 204), this, FORWARD, oComm);
    ...
    randomizeParametersComboPanel.setLayout(
        new BoxLayout(randomizeParametersComboPanel, BoxLayout.X_AXIS));
  ...
  public void randomizeParameters(RandomizeParametersButton b) {
    if (b.seedDirection == 1) {
      ...
      if (currentRandomizerSeedIndex > GlobalConstants.randomizerSeedCount)
        currentRandomizerSeedIndex = 0;
    } else {
      currentRandomizerSeedIndex--;
      if (currentRandomizerSeedIndex < 0)
        currentRandomizerSeedIndex = GlobalConstants.randomizerSeedCount;
    }

    randomizerSeedNumber.setText(
        "Seed: " + Integer.toString(randomizerSeeds.get(currentRandomizerSeedIndex)));

    sliderRed.setValue(
        MathHelper.getStandardDistributionRandomNumber(
            -50, 50, 0, randomizerSeeds.get(currentRandomizerSeedIndex) + 0));
    sliderGreen.setValue(
        MathHelper.getStandardDistributionRandomNumber(
            -50, 50, 0, randomizerSeeds.get(currentRandomizerSeedIndex) + 100000));
    sliderBlue.setValue(
        MathHelper.getStandardDistributionRandomNumber(
            -50, 50, 0, randomizerSeeds.get(currentRandomizerSeedIndex) + 200000));
    sliderBrightness.setValue(
        MathHelper.getStandardDistributionRandomNumber(
            -50, 50, 0, randomizerSeeds.get(currentRandomizerSeedIndex) + 300000));
    sliderContrast.setValue(
        MathHelper.getStandardDistributionRandomNumber(
            -50, 50, 0, randomizerSeeds.get(currentRandomizerSeedIndex) + 300200));
    sliderSaturation.setValue(
        MathHelper.getStandardDistributionRandomNumber(
            -50, 50, 0, randomizerSeeds.get(currentRandomizerSeedIndex) + 500000));
    sliderDither.setValue(
        MathHelper.getStandardDistributionRandomNumber(
            0, 100, 0, randomizerSeeds.get(currentRandomizerSeedIndex) + 600000));
    sliderSharpness.setValue(
        MathHelper.getStandardDistributionRandomNumber(
            0, 100, 0, randomizerSeeds.get(currentRandomizerSeedIndex) + 700000));
