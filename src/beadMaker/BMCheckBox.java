...
public class BMCheckBox extends JCheckBox implements InterObjectCommunicatorEventListener {
  ...
  BMCheckBox(String myLabel, boolean checkedState, InterObjectCommunicator myOComm) {
    ...
    if (myLabel.equals("Perler")) {
      objectName = "BM_CHECKBOX_PERLER";
    }
    if (myLabel.equals("Artkal-S")) {
      objectName = "BM_CHECKBOX_ARTKAL";
    }
    if (myLabel.equals("Show Grid")) {
      objectName = "BM_CHECKBOX_SHOW_GRID";
    }
    if (myLabel.equals("Show Pixels as Beads")) {
      objectName = "BM_CHECKBOX_PIXELS_AS_BEADS";
    }
    if (myLabel.equals("Flip Image")) {
      objectName = "BM_CHECKBOX_FLIP_IMAGE";
    }
  ...
  public void onInterObjectCommunicator_CommunicateEvent(String descriptor, Object o) {
    if (o instanceof KeyEvent) {
      ...
      // ConsoleHelper.PrintMessage("fired a keyPressed event from oCommInterface in CHECKBOX" +
      // e.getKeyCode());
      // if the descriptor matches the button name (text) then do the stuff
      // ConsoleHelper.PrintMessage("descriptor = " + descriptor + ", this.getText() = " +
      // this.getText());
